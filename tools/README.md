...
```sh
tools/render-all.sh          # uses $SOUNDFONT, else the autodetected soundfont
```

The committed piano and `*.choir.mp3` files were produced this way with the
GeneralUser GS soundfont. FluidSynth is deterministic, so re-running with the
same soundfont reproduces them byte-for-byte; a different soundfont yields
slightly different audio. (The `*.choir-chorium.mp3` comparison renders are
exploration artifacts and are not regenerated by `render-all.sh`.)
