...
% Likewise \vsize: plain TeX's default of 8.9in left a ~45mm bottom margin on
% A4 (vs. the 25.4mm top), leaving the page more room than it needed to break
% into. \vsize = pageheight - 2in makes the bottom margin 1 inch too.
...
% \title{...}: a hymn heading.  Bold Helvetica, centered (like the document
% caption above it, set with \headfont in makrina.tex) rather than flush left
% with the hymn text below it, with a little air above (separating it from
% whatever came before) and below (before the hymn's own line spacing, sized
% for the pitch marks, takes over).  \centerline avoids \everypar's hanging
% indent, which is for hymn paragraphs, not headings.
\font\titlefont=phvb8t scaled 1600
\def\title#1{\par\vskip6pt\centerline{\titlefont#1}\vskip5pt}
